...
1. **Visual steering** — a target presented left/right causes the simulated MicroDuck to turn in the correct direction.
2. **Looming stop** — an approaching obstacle causes a safe stop before a predefined safety boundary.

The first vertical slice is visual steering. Looming is added only after the steering slice passes its integration gate.
...
4. Real/simulated camera target -> steering end to end.
5. Looming feature -> validated neural path -> stop end to end.
...
- [x] first behavior is visual steering,
