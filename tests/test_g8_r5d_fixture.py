...
from scripts.g8_r5d_trial import LoomingChain, acknowledged_precondition_move
...
class G8R5dFixtureTests(unittest.TestCase):
    ...
    def test_post_ack_zero_move_is_rejected_and_stops_refresh(self):
        adapter = Adapter()
        fixture = IsolatedStopPublisher(adapter)
        fixture.send(minted(stop=True))
        fixture.send(minted(stop=True, sequence=2))
        with self.assertRaises(UnexpectedNonzeroOutput):
            fixture.send(minted(sequence=3))
        self.assertEqual(len(adapter.calls), 2)
        self.assertEqual(fixture.post_stop_move_count, 1)

    def test_forged_mapping_rejected(self):
        with self.assertRaises(TypeError):
            IsolatedStopPublisher(Adapter()).send({"intent": {"stop": False}})

    def test_fixture_scheduler_keeps_refreshing_until_complete(self):
        wd = ControllerWatchdog(ROOT / "config/watchdog_v1.json")
        ...
        import threading
        timer = threading.Timer(.18, scheduler.request_complete)
        timer.start()
        ...
        self.assertTrue(any(output["watchdog_state"] != "healthy" and output["intent"]["stop"]
