...
from scripts import g8_r5d_batch as batch


class G8R5dBatchTests(unittest.TestCase):
    def run_fake_batch(self, *, trial_exit=0, trial_summary="{invalid", trial_raises=False):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / "config").mkdir()
            (root / "config/g8_r5d_stop_refresh_v1.json").write_text(json.dumps({
            ...
            output = root / "output"
            argv = ["g8_r5d_batch.py", "--root", str(root), "--microduck", str(microduck),
