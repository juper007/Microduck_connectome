...
class G8R5cMetricsTests(unittest.TestCase):
    ...
    def test_state_machine_requires_order_and_complete_path_for_pass(self):
        full = ["SETUP", "MOTION_PRECONDITION", "MOTION_CONFIRMED",
                ...
                "ROBOT_STOP_SENT", "ROBOT_STOP_ACK", "STOP_REFRESHING", "MOTION_STOPPED", "COMPLETE"]
...
class StopRefreshCadenceTests(unittest.TestCase):
    def test_ack_gaps_and_tail_must_meet_frozen_bound(self):
        rows = [{"sent_at_ns": 0, "ack_at_ns": 2_000_000},
                {"sent_at_ns": 20_000_000, "ack_at_ns": 22_000_000},
                {"sent_at_ns": 40_000_000, "ack_at_ns": 42_000_000}]
        ok = stop_refresh_cadence(rows, maximum_gap_ms=100,
                                  deadman_timeout_ms=500,
                                  stopped_confirmed_ns=61_000_000)
        self.assertTrue(ok["valid"])
        self.assertEqual(ok["max_gap_ms"], 20)
        self.assertFalse(stop_refresh_cadence(rows, maximum_gap_ms=10,
                                              deadman_timeout_ms=500,
