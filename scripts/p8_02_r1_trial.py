...
def validate_frozen_selection(protocol):
    ...
    versions = {
        ...
        "p8-02-final-approach-v1": (list(range(880000, 880020)), "fractional_rgb_v21"),
    }
    ...
    if version in ("p8-r3-v24-official-development-v1", "p8-02-final-approach-v1"):
    ...
    if (not isinstance(runs, list) or len(runs) != len(versions[version][0])
            or [row.get("seed") for row in runs] != versions[version][0]
            or len({row.get("run_id") for row in runs}) != len(runs)):
        raise RuntimeError("P8-R3 official three-reset seed matrix mismatch")
    if version == "p8-02-final-approach-v1":
        expected = [(f"A{i:02d}", seed, (2.0, 2.6, 3.0)[i % 3])
                    for i, seed in enumerate(versions[version][0])]
        if ([(row.get("run_id"), row.get("seed"), row.get("arm_elapsed_s"))
             for row in runs] != expected
                or protocol.get("scenario_seeds") != versions[version][0]
                or protocol.get("arm_elapsed_s") != [row[2] for row in expected]
                or protocol.get("required_successful_independent_trials") != 19):
            raise RuntimeError("P8-02 final 20-run matrix or threshold changed")
    if version in ("p8-r3-v22-official-development-v1",
                   "p8-r3-v23-official-development-v1",
                   "p8-r3-v24-official-development-v1"):
...
def validate_frozen_material(root: Path, protocol: dict) -> None:
    ...
    root = root.resolve()
    if protocol["schema_version"] == "p8-02-final-approach-v1":
        final_path = (root / protocol["final_protocol_path"]).resolve()
        if not final_path.is_relative_to(root / "config"):
            raise RuntimeError("final preregistration path escapes config")
        ...
        if (final.get("schema_version") != "p8-v2-final-protocol-v1"
                or [(r["run_id"], r["seed"], r["arm_elapsed_s"])
                    for r in protocol["ordered_official_runs"]]
                   != [(r["trial_id"], r["seed"], r["arm_elapsed_s"])
                       for r in expected_runs]
                or protocol["selected_v2"] != final["selected_pipeline"]["looming_estimator"]
                or protocol["graph_sha256"] != final["selected_pipeline"]["graph_v2_sha256"]
                or protocol["visual_hz"] != final["selected_pipeline"]["visual_hz"]):
            raise RuntimeError("final approach execution differs from preregistration")
    ...
    if protocol["schema_version"] in ("p8-r3-v24-official-development-v1",
                                       "p8-02-final-approach-v1"):
...
def write_arm_marker(path: Path, trial_id: str, seed: int, armed_ns: int) -> None:
    """Durable arm evidence before any neural observation worker can publish motion."""
    if path.exists():
        raise RuntimeError("armed marker already exists")
    payload = json.dumps({"schema_version": "p8-02-r1-armed-v1", "trial_id": trial_id,
                          "seed": seed, "armed_ns": armed_ns}, sort_keys=True).encode() + b"\n"
...
def append_progress(path: Path, state: str, when_ns: int) -> None:
    payload = json.dumps({"state": state, "timestamp_ns": when_ns}, sort_keys=True).encode() + b"\n"
...
def validate_r1_selection(protocol: dict):
    if protocol.get("schema_version") != "p8-02-r1-execution-v1":
        raise RuntimeError("R1 execution schema mismatch")
    stage = protocol.get("r1_stage")
    expected = ([(f"D{i:02d}", 886020+i, [2.0,2.6,3.0][i]) for i in range(3)]
                if stage == "D" else
                [(f"B{i:02d}", 880020+i, [2.0,2.6,3.0][i%3]) for i in range(20)]
                if stage == "B" else None)
    if expected is None or [(r.get("run_id"),r.get("seed"),r.get("arm_elapsed_s"))
                             for r in protocol.get("ordered_official_runs",[])] != expected:
        raise RuntimeError("R1 frozen run matrix mismatch")
    if protocol.get("scenario_seeds") != [r[1] for r in expected]:
        raise RuntimeError("R1 seed guard mismatch")
    if (protocol.get("arm_elapsed_s") != [r[2] for r in expected]
            or protocol.get("maximum_neural_observation_ms_by_run") !=
            [{2.0: 1000, 2.6: 750, 3.0: 550}[r[2]] for r in expected]
            or protocol.get("required_successful_independent_trials") != (3 if stage == "D" else 19)):
        raise RuntimeError("R1 arm, observation, or success threshold mismatch")
    inherited = dict(protocol)
    inherited["schema_version"] = "p8-r3-v24-official-development-v1"
    inherited["ordered_official_runs"] = [dict(run_id=f"{i+1:02d}-{885721+i}",seed=885721+i,
                                               arm_elapsed_s=[2.0,2.6,3.0][i]) for i in range(3)]
    inherited["scenario_seeds"] = [885721,885722,885723]
    inherited["arm_elapsed_s"] = [2.0,2.6,3.0]
    inherited["required_successful_independent_trials"] = 3
    return validate_frozen_selection(inherited)


def validate_r1_material(root: Path, protocol: dict) -> None:
    base = json.loads((root / "config/p8_02_final_approach_v1.json").read_text())
    ...
    runs = (r1["development_gate"]["ordered_runs"] if protocol["r1_stage"] == "D"
            else r1["ordered_official_runs"])
    if [(r["trial_id"],r["seed"],r["arm_elapsed_s"]) for r in runs] != [
        (r["run_id"],r["seed"],r["arm_elapsed_s"]) for r in protocol["ordered_official_runs"]]:
        raise RuntimeError("R1 execution differs from prospective protocol")
...
def verify_protocol(root, protocol_path, protocol, args):
    ...
    estimator_config, visual_hz = validate_r1_selection(protocol)
    if protocol["schema_version"] == "p8-02-r1-execution-v1" and (
    ...
    validate_r1_material(root, protocol)
    frozen_folder = Path(protocol["frozen_output_dir"]) / protocol["ordered_official_runs"][args.trial_index]["run_id"] / f"attempt-{args.attempt:02d}"
    if any(path.resolve().parent != frozen_folder.resolve()
           for path in (args.raw, args.events, args.ledger, args.visual, args.summary,
                        args.policy_readback, args.armed_marker, args.progress)):
...
def run(args):
    ...
    if not 0 <= args.trial_index < len(protocol.get("ordered_official_runs", [])):
        raise ValueError("trial index outside frozen protocol")
    ...
    seed = row["seed"]
    trial_id = f"p8-02-r1-{row['run_id']}"
    ...
    def transition(state, when_ns):
        with event_lock:
            ...
            append_progress(args.progress, state, when_ns)
    ...
    try:
        ...
        write_arm_marker(args.armed_marker, trial_id, seed, phase_started_ns)
    ...
    if protocol["schema_version"] in ("p8-r3-v22-official-development-v1",
                                       "p8-r3-v23-official-development-v1",
                                       "p8-r3-v24-official-development-v1",
                                       "p8-02-final-approach-v1", "p8-02-r1-execution-v1"):
    ...
    summary = {
        ...
        "evidence_role": protocol.get("evidence_role", "development_probe"),
...
def main():
    ...
    ap.add_argument("--armed-marker", type=Path, required=True)
