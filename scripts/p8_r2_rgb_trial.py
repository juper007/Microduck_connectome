"""P8-R2 paired RGB development comparison on official Thor."""
...
def verify_protocol(root, protocol_path, protocol, args):
    ...
    scenario_info = protocol["scenario_configs"][representation]
    scenario_path = root / scenario_info["path"]
    if sha(scenario_path) != scenario_info["git_blob_sha256"]:
...
def run(args):
    ...
    row = protocol["ordered_official_runs"][args.trial_index]
    seed = row["seed"]
    trial_id = f"p8-r2-dev-{row['run_id']}"
    ...
    result = "PASS" if all(transport_checks.values()) and not observer_errors else "FAIL"
    behavior_result = "PASS" if result == "PASS" and checks["trigger_before_frozen_boundary"] else "FAIL"
    ...
    summary = {
        ...
        "representation": row["representation"],
        "scenario_config_sha256": protocol["scenario_configs"][row["representation"]]["git_blob_sha256"],
