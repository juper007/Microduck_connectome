"""Run preregistered independent P7-03 controls on official Thor robotd/MuJoCo."""
...
from scripts.p7_03_preregister import SOURCE_SHA256, generate
...
def validate_control_specs(experiment: dict) -> list[dict]:
    ...
    if (experiment["schema_version"] != "steering-experiment-v4"
            ...
            or len(specs) != 40):
        raise ValueError("P7-03 no-target trial count or schema mismatch")
    ...
    for index, spec in enumerate(specs, 1):
        if (spec["trial_id"] != f"p7-03-no-target-{index:03d}"
...
def summarize(rows: list[dict], experiment: dict, *, head: str,
    ...
    return {
        "schema_version": "p7-03-no-target-batch-v1",
        "execution_target": "Thor",
        "hostname": socket.gethostname(),
        "started_utc": started_utc,
        "ended_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "source_head": head,
        "source_manifest_sha256": experiment["p7_03"]["source_manifest_sha256"],
        "reused_target_summary_sha256": experiment["p7_03"]["target_evidence_reused"]["summary_sha256"],
...
def validate_reused_target_evidence(experiment: dict) -> dict:
    """Require the exact merged P7-02 target evidence before controls run."""
    reference = experiment["p7_03"]["target_evidence_reused"]
    for name in ("summary", "journal"):
        if sha256_file(Path(reference[f"{name}_path"])) != reference[f"{name}_sha256"]:
            raise ValueError(f"reused P7-02 target {name} hash mismatch")
    target = json.loads(Path(reference["summary_path"]).read_text(encoding="utf-8"))
    if (target["result"] != "PASS"
            or target["evaluated_valid_target_trials"] != 120
            or target["counts"]["correct"] != 108
            or target["safety_limit_violations"] != 0
            or target["source_head"] !=
            "fdde4578f0705a1a30cfe1c2ea5d5e48faf0d2aa"
            or target["experiment_sha256"] != SOURCE_SHA256
            or target["walking_policy_sha256"] !=
            experiment["walking_policy"]["sha256"]):
        raise ValueError("reused P7-02 target evidence mismatch")
    return target
...
def main() -> None:
    ...
    if (not socket.gethostname().startswith("jetsonthor")
        ...
        raise RuntimeError("final P7-03 control batch requires Thor Python 3.12")
    root = args.root.resolve()
    experiment_path = args.experiment.resolve()
    tracked_manifest = (root / "config/steering_no_target_p7_03_v1.json").resolve()
    if experiment_path != tracked_manifest:
        raise RuntimeError("final batch requires tracked P7-03 manifest")
    ...
    committed_manifest = subprocess.check_output(
        ["git", "-C", str(root), "show", "HEAD:config/steering_no_target_p7_03_v1.json"])
    if experiment_path.read_bytes() != committed_manifest:
        raise RuntimeError("P7-03 manifest differs from committed bytes")
    experiment = json.loads(experiment_path.read_text(encoding="utf-8"))
    source_manifest_path = root / "config/steering_experiment_v4.json"
    if sha256_file(source_manifest_path) != SOURCE_SHA256:
        raise RuntimeError("P7-02 source manifest hash mismatch")
    original = json.loads(source_manifest_path.read_text(encoding="utf-8"))
    if experiment != generate(original, source_sha256=SOURCE_SHA256):
        raise RuntimeError("P7-03 manifest changed frozen controller, policy, rules, or seeds")
    validate_reused_target_evidence(experiment)
    specs = validate_control_specs(experiment)
    fixture_path = Path(__file__).resolve()
    if fixture_path != (root / "scripts/p7_03_no_target_batch.py").resolve():
        raise RuntimeError("final batch requires tracked no-target harness")
    committed_fixture = subprocess.check_output(
        ["git", "-C", str(root), "show", "HEAD:scripts/p7_03_no_target_batch.py"])
        ...
        for index, spec in enumerate(specs, 1):
            ...
            if acquisition is not None and acquisition["accepted"]:
                command = [
                    ...
                    "--run-id", f"p7-03-no-target-{index:03d}",
