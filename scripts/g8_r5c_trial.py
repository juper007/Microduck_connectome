...
def verify_protocol(root, protocol_path, protocol, args):
    ...
    if protocol["schema_version"] != "g8-r5c-isolated-neural-stop-protocol-v1":
    ...
    if not args.development_probe:
        committed = subprocess.check_output(["git", "-C", str(root), "show",
                                             "HEAD:config/g8_r5c_neural_stop_v1.json"])
...
def run(args):
    ...
    trial_id = f"g8-r5c-{args.trial_index + 1:02d}-{seed}"
    ...
    def record_geometry(pose, sample_ns, phase):
        if initial_pose is None or chain.trial is None or chain.started_ns is None:
            return
        elapsed_s = protocol["scenario_elapsed_start_s"] + max(
            0.0, (sample_ns - chain.started_ns) / 1e9)
        if post_ack_virtual_center and sample_ns >= post_ack_virtual_center["frozen_at_ns"]:
            center_x, center_y = post_ack_virtual_center["x_m"], post_ack_virtual_center["y_m"]
            target_mode = "held_evaluation_only_after_visual_termination"
            elapsed_s = post_ack_virtual_center["scenario_elapsed_s"]
        else:
            center_x, center_y = sphere_center(scenario, chain.trial, elapsed_s)
            target_mode = "original_approaching_visual_scenario"
        ...
        if row["sphere_surface_clearance_m"] <= protocol["minimum_center_clearance_after_ack_m"]:
            raise RuntimeError("virtual sphere clearance breached frozen fixture margin")
    ...
    observer = threading.Thread(target=observer_worker, name="g8-r5c-observer")
    observer.start()
    ...
    def observe(update, output, transport):
        ...
        if transport == "robot_stop_refreshed":
            if chain.started_ns is not None and chain.trial is not None:
                held_elapsed_s = protocol["scenario_elapsed_start_s"] + max(
                    0.0, (ack_ns - chain.started_ns) / 1e9)
                held_x, held_y = sphere_center(scenario, chain.trial, held_elapsed_s)
                post_ack_virtual_center.update({"x_m": held_x, "y_m": held_y,
                                                "frozen_at_ns": ack_ns,
                                                "scenario_elapsed_s": held_elapsed_s})
                events.append({"timestamp_ns": ack_ns, "kind": "visual_scenario_terminated_at_stop_ack",
                               "held_evaluation_center_x_m": held_x,
                               "held_evaluation_center_y_m": held_y,
                               "no_further_controller_visual_input": True})
            scheduler.request_handoff()
    ...
    try:
        ...
        for index in range(round(pre["duration_s"] * 1000 / pre["command_period_ms"])):
            ...
            move_result, move_call_ns, move_write_ns, move_ack_ns = acknowledged_precondition_move(
        ...
        center_x, center_y = sphere_center(scenario, chain.trial,
                                           protocol["scenario_elapsed_start_s"])
        ...
        scheduler_result = scheduler.run(remaining_neural_s)
        pending.join()
        ...
        if state_path[-1] != "ROBOT_STOP_ACK" or not scheduler._fixture_handoff.is_set():
        ...
        stop_events_before_stage2 = [e for e in events if e["kind"] == "control_publish"
                                     and e["transport_action"] == "robot_stop_refreshed"]
        if len(stop_events_before_stage2) != 1:
            raise RuntimeError("another robot-facing stop was sent before read-only handoff")
        ...
        if not post_ack_virtual_center or post_ack_virtual_center["frozen_at_ns"] != stop_ack:
            raise RuntimeError("visual scenario was not terminated at first stop ACK")
        old_deadline_ns = last_motion["request_call_ns"] + int(protocol["deadman_timeout_ms"] * 1e6)
        new_deadline_ns = stop_call + int(protocol["deadman_timeout_ms"] * 1e6)
        ...
        while time.monotonic_ns() < post_deadline_ns:
            ...
            if any("deadman" in str(reason).lower() for reason in limited_by):
                ...
                if received_ns < new_deadline_ns:
                    raise RuntimeError("deadman intervention preceded new stop deadline")
            if first_applied_reduction_ns is None and applied_vx <= reduction_limit:
                first_applied_reduction_ns = received_ns
                if first_applied_reduction_ns >= old_deadline_ns:
                    raise RuntimeError("applied-vx reduction did not precede old deadman deadline")
            if (first_applied_near_zero_ns is None
                ...
                if first_applied_near_zero_ns >= new_deadline_ns:
    ...
    post_ack_commands = [e for e in events if e["kind"] == "control_publish"
    ...
    checks = {
        ...
        "deadman_excluded": deadman["valid"] and last_move_ack_to_stop_ack_ms is not None
                            ...
                            and refreshed_deadline_ns is not None
        ...
        "no_post_ack_transport_before_complete": not post_ack_commands,
    ...
    summary = {
        "schema_version": "g8-r5c-isolated-neural-stop-trial-v1", "result": result,
