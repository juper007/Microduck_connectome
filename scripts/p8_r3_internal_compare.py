"""P8-R3 development RGB-to-graph-v2 comparison; no robotd or final seeds.
...
SEEDS = (885301, 885302, 885303)  # Development only; disjoint from P8 final.
...
METHODS = {
    ...
    "B_relative_radius": LoomingV2Config(method="relative_radius", full_scale_rate_per_s=0.25),
    "C_log_area_regression": LoomingV2Config(method="log_area_regression", full_scale_rate_per_s=0.5),
}
...
def run_case(graph, scenario, *, method, config, seed, arm_elapsed_s, motion):
    ...
    for index in range(step_count):
        ...
        if index % 5 == 0:
            pixels = render_pixels(scenario, trial, pose=POSE, elapsed_s=elapsed_s)
    ...
    return {
        "method": method, "seed": seed, "arm_elapsed_s": arm_elapsed_s, "motion": motion,
        ...
        for width, height in RESOLUTIONS:
            ...
            for motion in MOTIONS:
                for seed, arm_elapsed_s in zip(SEEDS, ARM_ELAPSED_S):
                    cases.append(run_case(graph, scenario, method=method, config=config,
                                          seed=seed, arm_elapsed_s=arm_elapsed_s, motion=motion))
    report = {
        "schema_version": "p8-r3-internal-development-v1",
        "scope": "deterministic synthetic RGB and graph-v2; no moving body or robotd",
        "development_only": True, "seeds": SEEDS, "arm_elapsed_s": ARM_ELAPSED_S,
        "visual_hz": 10, "neural_hz": 50,
