"""P8-R3 V2.2 prospective 20 Hz paired screen; no robotd/final seeds.
...
from microduck_connectome.looming_scenario import evaluator_truth, load_config, make_trial, pixels_sha256
...
SEEDS = (885501, 885502, 885503)  # Fresh V2.2 internal development seeds.
...
def run_case(graph, scenario, *, visual_hz, method, config, seed, arm_elapsed_s, motion):
    ...
    frames, steps = [], []
    current = None
    next_camera_ns = 20_000_000
    camera_period_ns = 1_000_000_000 // visual_hz
    camera_frame_id = 0
    ...
    for index in range(step_count):
        ...
        if now_ns >= next_camera_ns:
            camera_frame_id += 1
            camera_elapsed_s = arm_elapsed_s + (next_camera_ns - 20_000_000) / 1e9
            pixels = render_fractional_pixels(scenario, trial, pose=POSE,
                                              elapsed_s=camera_elapsed_s)
            current = chain.pipeline.process(
                pixels, camera_timestamp_ns=next_camera_ns, camera_frame_id=camera_frame_id,
                tof_left_mm=scenario["tof_mm"], tof_center_mm=scenario["tof_mm"],
                tof_right_mm=scenario["tof_mm"], tof_timestamp_ns=next_camera_ns,
                tof_frame_id=camera_frame_id, now_ns=now_ns,
            )
            camera_truth = evaluator_truth(scenario, trial, pose=POSE, elapsed_s=camera_elapsed_s)
            frames.append({
                "elapsed_s": camera_elapsed_s, "timestamp_ns": next_camera_ns,
                "processed_at_ns": now_ns,
                "processing_delay_ms": (now_ns - next_camera_ns) / 1e6,
                ...
                "distance_to_boundary_m": camera_truth["distance_to_boundary_m"],
            })
        ...
        steps.append({
            ...
            "frame_timestamp_ns": current["timestamp_ns"],
            "frame_age_ms": (now_ns - current["timestamp_ns"]) / 1e6,
    ...
    return {
        "representation": "fractional", "visual_hz": visual_hz,
        ...
        "maximum_frame_age_ms": max(row["frame_age_ms"] for row in steps),
        "maximum_frame_processing_delay_ms": max(row["processing_delay_ms"] for row in frames),
        "all_neural_steps_healthy": all(row["runtime_healthy"] and
                                        row["watchdog_state"] == "healthy" for row in steps),
...
def main():
    ...
    for method, config in METHODS.items():
        for visual_hz in RATES_HZ:
            for width, height in RESOLUTIONS:
                scenario = {**baseline, "image_width_px": width, "image_height_px": height}
                for motion in MOTIONS:
                    for seed, arm_elapsed_s in zip(SEEDS, ARM_ELAPSED_S):
                        cases.append(run_case(graph, scenario, visual_hz=visual_hz,
                                              method=method, config=config, seed=seed,
                                              arm_elapsed_s=arm_elapsed_s, motion=motion))
    report = {
        "schema_version": "p8-r3-v22-internal-development-v1",
        "scope": "prospective 20 Hz fractional RGB plus estimator A and graph-v2; paired fixed-pose virtual geometry, no moving body or robotd",
        "development_only": True, "seeds": SEEDS, "arm_elapsed_s": ARM_ELAPSED_S,
        "visual_hz": RATES_HZ, "neural_hz": 50,
