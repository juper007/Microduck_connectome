...
def score_attempt(folder: Path, expected: dict) -> dict:
    ...
    if (summary.get("trial_id") != f"p8-02-r1-{expected['trial_id']}"
            or summary.get("seed") != expected["seed"]
            or summary.get("arm_elapsed_s") != expected["arm_elapsed_s"]
            or summary.get("schema_version") != "p8-02-r1-trial-v1"):
...
def score_batch(root: Path, protocol: dict, journal: dict) -> dict:
    ...
    planned = (protocol["development_gate"]["ordered_runs"] if stage == "D"
               else protocol["ordered_official_runs"] if stage == "B" else None)
    count = 3 if stage == "D" else 20
    prefix, seed_start = ("D",886020) if stage == "D" else ("B",880020)
    if (planned is None or len(planned) != count
            or [(r["trial_id"],r["seed"]) for r in planned]
            != [(f"{prefix}{i:02d}",seed_start+i) for i in range(count)]):
        raise ValueError("R1 fixed seed matrix mismatch")
    assigned = journal.get("ids", [])
    if len(assigned) != count:
        raise ValueError("journal denominator mismatch")
    attempts = []
    for row, state in zip(planned, assigned):
        if row["trial_id"] != state.get("trial_id") or row["seed"] != state.get("seed"):
            raise ValueError("journal ID/seed mismatch")
        history = state.get("attempts", [])
        armed = [a for a in history if a.get("armed") is True]
        if len(armed) == 1:
            ...
            try:
                ...
                result = {"trial_id":row["trial_id"],"seed":row["seed"],"success":False,
                          ...
                          "failure_causes":[f"raw_extract_exception:{type(error).__name__}:{error}"]}
            if len(history)>3 or any(a.get("armed") for a in history[:-1]):
                result["success"]=False
                result["failure_causes"].append("invalid_retry_history")
            attempts.append(result)
        else:
            attempts.append({"trial_id":row["trial_id"],"seed":row["seed"],
                             "success":False,"raw_accounted":False,
                             "safety_limit_violations":None,"latencies_ms":{},
                             "failure_causes":["missing_or_multiple_armed_attempts"]})
    successes=sum(a["success"] for a in attempts)
    accounted=all(a["raw_accounted"] for a in attempts)
    zero_safety=all(a["safety_limit_violations"]==0 for a in attempts)
    latencies={name:timing([a["latencies_ms"][name] for a in attempts
                            if name in a["latencies_ms"]])
               for name in ("looming_to_neural_stop","neural_stop_to_request",
                            "neural_stop_to_ack","ack_to_stopped","looming_to_stopped")}
    confirmed=sum((a.get("margins_m",{}).get("stopped_confirmation") or -1)>0
                  for a in attempts)
    return {"schema_version":"p8-02-r1-score-v1","stage":stage,"planned":count,
            "successes":successes,"success_rate":successes/count,
            "all_raw_accounted":accounted,"zero_safety_limit_violations":zero_safety,
            "wilson_95_ci":wilson(successes,count),
            "confirmed_preboundary_stops":confirmed,
            "confirmed_preboundary_wilson_95_ci":wilson(confirmed,count),
            "latencies":latencies,"trials":attempts,
            "result":"PASS" if successes >= (3 if stage=="D" else 19)
            and accounted and zero_safety else "FAIL"}
