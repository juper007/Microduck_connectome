"""Six fresh official Thor resets for the frozen P8-R2 RGB comparison."""
...
def main():
    ...
    if protocol_path.read_bytes() != subprocess.check_output([
        "git", "-C", str(root), "show", "HEAD:config/p8_r2_rgb_resolution_dev_v2.json"]):
    ...
    try:
        if len(planned) != 6 or [r["seed"] for r in planned] != protocol["scenario_seeds"]:
            raise ValueError("frozen six-row seed matrix mismatch")
        for index, planned_row in enumerate(planned):
            ...
            row = {"trial_index": index, "seed": seed,
                   ...
                   "representation": planned_row["representation"],
                   "scenario_sha256": protocol["scenario_configs"][planned_row["representation"]]["git_blob_sha256"],
                   "trial_started": False,
                   "pretrial_logs": {}}
            rows.append(row)
            try:
                ...
                command = [
                    sys.executable, str(root / "scripts/p8_r2_rgb_trial.py"),
    ...
    for unstarted in planned[len(rows):]:
        rows.append({"run_id": unstarted["run_id"], "seed": unstarted["seed"],
                     "representation": unstarted["representation"],
                     "scenario_sha256": protocol["scenario_configs"][unstarted["representation"]]["git_blob_sha256"],
                     "trial_started": False, "failure": "unstarted_after_prior_failure"})
    candidate_rows = [row for row in rows if row["representation"] == "candidate"]
    candidate_primary_passes = sum(
        row.get("summary", {}).get("r2_primary_screen_result") == "PASS"
        for row in candidate_rows)
    result = "PASS" if (batch_error is None
                            and len(rows) == len(protocol["ordered_official_runs"])
                            and len(candidate_rows) == 3
                            and candidate_primary_passes == 3
    ...
    report = {
        "schema_version": "p8-r2-rgb-resolution-development-batch-v2",
        ...
        "candidate_primary_passes": candidate_primary_passes,
        "candidate_primary_planned": 3,
    ...
    manifest_path.write_text(json.dumps({
        "schema_version": "p8-r2-raw-manifest-v2", "source_head": head,
