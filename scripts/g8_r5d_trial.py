...
def verify_protocol(root, protocol_path, protocol, args):
    ...
    if protocol["schema_version"] != "g8-r5d-production-stop-refresh-protocol-v1":
    ...
    if not args.development_probe:
        committed = subprocess.check_output(["git", "-C", str(root), "show",
                                             "HEAD:config/g8_r5d_stop_refresh_v1.json"])
...
def run(args):
    ...
    trial_id = f"g8-r5d-{args.trial_index + 1:02d}-{seed}"
    ...
    scheduler = ProductionStopRefreshScheduler(
    ...
    summary = {
        "schema_version": "g8-r5d-production-stop-refresh-trial-v1", "result": result,
